...
export function renderHtml(report) {
  ...
  const evidenceRows = (report.semantic?.sessions ?? []).map((session) => `<tr><td>${escapeHtml(session.id)}</td><td>${escapeHtml(session.source)}</td><td>${escapeHtml(session.date ?? 'unknown')}</td></tr>`).join('');
  const evidenceIndex = `<section><h2>Evidence index</h2><div class="table-wrap"><table><thead><tr><th>Opaque reference</th><th>Agent</th><th>Date</th></tr></thead><tbody>${evidenceRows || '<tr><td colspan="3">No semantic evidence sessions.</td></tr>'}</tbody></table></div></section>`;
